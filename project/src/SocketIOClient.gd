...
func _init(url: String, auth: Variant=null) -> void:
	_auth = auth
...
func _engineio_decode_packet(packet: String) -> void:
...
func _engineio_send_packet(type: EngineIOPacketType, payload: String="") -> void:
	if len(payload) == 0:
		_client.send_text("%d" % type)
	else:
		_client.send_text("%d%s" % [type, payload])

func _socketio_parse_packet(payload: String) -> void:
